	...
	return form.messages.map((message, id) => {
		return {
			...
			props: {
				...
				setMessage: (message: DiscordWebhookMessage) =>
...
const addMessage = () => {
	form.messages.push({
		...
		avatar_url:
			webhook.value?.avatar ??
			"http://localhost:3333/images/tools/discord/default_avatar.png",
	});
...
const onSwitchTheme = (t: ThemeType) => {
	...
	const discordTheme = themes.find(
		(discordTheme) => discordTheme.name === theme.current.name
	);
	if (discordTheme) {
			...
			<div class="grid grid-cols-2 gap-8 pt-12">
				...
				<div>
					...
					>
						<template #header>
							<div
								class="flex items-center justify-between border-b p-8 dark:border-b-primary-800"
							>
								<div>
								...
								<UIModal
									...
									:description="
										$t('tools.global.theme.description')
									"
									ref="themeModal"
									:onApply="
										() => onSwitchTheme(theme.selected)
									"
								>
						...
						>
							<ToolsDiscordEmbedMakerPreview" v-for="(message,
							key) in form.messages" :key="key" :message="message"
							:isDark="theme.current.name === 'Default' ?
							colorMode.value === 'dark' : theme.current.isDark"
							/>
						</TransitionGroup>
						<template #footer>
							<div
