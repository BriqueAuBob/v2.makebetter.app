...
<template>
	<div
		...
		:style="`border-color: ${embed.color}; max-width: ${
			embed?.image?.url ? '432' : '516'
		}px; background-color: ${isDark ? '#2B2D31' : '#F2F3F5'};`"
	>
		<div
			v-if="
				embed.author?.name ||
		...
		>
			...
			<span class="text-xs text-gray-400">
				...
				<span class="mx-1" v-if="embed.timestamp && embed.footer?.text"
					>•</span
				>
